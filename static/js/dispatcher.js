...
function di18n(key, fallback) {
    return (window.TETATET_I18N && window.TETATET_I18N[key]) || fallback;
}

function di18nf(key, fallback, vars = {}) {
    let text = di18n(key, fallback);
...
async function runDispatchAction(btn) {
    ...
    if (action === 'accept') {
        ...
        if (!sel?.value) {
            alert(di18n('dispatch_alert_select_driver', 'Выберите водителя'));
            return;
        }
        ...
        if (!confirm(di18nf('dispatch_error_cancel_confirm', 'Отменить заказ №{id}?', { id }))) return;
    ...
    try {
        if (typeof dispatchOrder !== 'function') {
            alert(di18n('dispatch_error_script_not_loaded', 'Скрипт не загружен. Нажмите Ctrl+F5.'));
            return;
        }
        ...
        alert(di18n('dispatch_error_action_failed', 'Ошибка при выполнении действия'));
    } finally {
        btn.disabled = false;
...
class DispatchBoard {
    ...
    async reload() {
        try {
            ...
            if (!res.ok) {
                this._showError(di18nf('dispatch_error_reload_failed', 'Не удалось обновить (ошибка {status}). Показаны данные с сервера.', { status: res.status }));
                return;
            }
            ...
            this._showError(di18n('dispatch_error_network', 'Ошибка сети. Показаны данные с сервера.'));
        }
    }
    ...
    render() {
        Object.entries(COL_MAP).forEach(([status, colId]) => {
            ...
            col.innerHTML = orders.length
                ...
                : `<p class="dispatch-empty">${di18n('dispatch_empty', 'Нет заказов')}</p>`;
        ...
        set('statArrived', this._ordersForStatus('arrived').length);
        set('statOnWay', this._ordersForStatus('on_way').length);
        set('statDone', this._ordersForStatus('done').length);
        set('statCancelled', this._ordersForStatus('cancelled').length);
    }
    ...
    _cardHtml(order) {
        ...
        const tariff = escapeHtml(labels[order.tariff] || order.tariff || '—');
        const route = `
            ...
            <div class="dispatch-card is-new" data-order-id="${order.id}">
                ...
                <div class="dispatch-actions">
                    <select class="driver-select" data-for="${order.id}">
                        <option value="">${di18n('dispatch_btn_select_driver', 'Выберите водителя')}</option>
                        ${driverOpts}
                    </select>
                    <button type="button" class="btn-dispatch btn-dispatch-accept" data-action="accept" data-id="${order.id}">${di18n('dispatch_btn_assign', 'Назначить водителя')}</button>
                    <button type="button" class="btn-dispatch btn-dispatch-cancel" data-action="cancel" data-id="${order.id}">${di18n('dispatch_btn_reject', 'Отклонить')}</button>
                </div>
            </div>`;
        }
        ...
        if (order.status === 'accepted') {
            ...
            <div class="dispatch-card" data-order-id="${order.id}">
                ...
                <div class="dispatch-card-price-row">${price}</div>
                <div class="dispatch-actions">
                    <button type="button" class="btn-dispatch btn-dispatch-go" data-action="arrived" data-id="${order.id}">${di18n('dispatch_btn_arrived', 'На месте')}</button>
                    <button type="button" class="btn-dispatch btn-dispatch-cancel" data-action="cancel" data-id="${order.id}">${di18n('dispatch_btn_cancel', 'Отменить')}</button>
                </div>
            </div>`;
        }
        ...
        if (order.status === 'arrived') {
            return `
            <div class="dispatch-card" data-order-id="${order.id}">
                <div class="dispatch-card-top"><strong>#${order.id}</strong><span>${di18n('dispatch_status_arrived', 'На месте')}</span></div>
                ${route}
                <div class="dispatch-card-meta">
                    <span>🚕 ${escapeHtml(order.driver_name || '—')}</span>
                    <a href="tel:${escapeHtml(order.phone)}" class="dispatch-phone">${escapeHtml(order.phone)}</a>
                ...
                <div class="dispatch-card-price-row">${price}</div>
                <div class="dispatch-actions">
                    <button type="button" class="btn-dispatch btn-dispatch-go" data-action="on_way" data-id="${order.id}">${di18n('dispatch_btn_start_trip', 'Поездка началась')}</button>
                    <button type="button" class="btn-dispatch btn-dispatch-cancel" data-action="cancel" data-id="${order.id}">${di18n('dispatch_btn_cancel', 'Отменить')}</button>
                </div>
            </div>`;
        }
        ...
        if (order.status === 'on_way') {
            ...
            <div class="dispatch-card" data-order-id="${order.id}">
                ...
                <div class="dispatch-actions">
                    <button type="button" class="btn-dispatch btn-dispatch-done" data-action="done" data-id="${order.id}">${di18n('dispatch_btn_complete', 'Завершить поездку')}</button>
                </div>
            </div>`;
        }
        ...
        if (order.status === 'done') {
            ...
            <div class="dispatch-card dispatch-card-done" data-order-id="${order.id}">
                <div class="dispatch-card-top"><strong>#${order.id}</strong><span>${di18n('dispatch_status_done', 'Завершён')}</span></div>
        ...
        if (order.status === 'cancelled') {
            return `
            <div class="dispatch-card dispatch-card-cancelled" data-order-id="${order.id}">
                <div class="dispatch-card-top"><strong>#${order.id}</strong><span class="dispatch-badge-cancelled">${di18n('dispatch_status_cancelled', 'Отменён')}</span></div>
                <div class="dispatch-card-route muted">
                    ${escapeHtml((order.from_address || '').slice(0, 40))} → ${escapeHtml((order.to_address || '').slice(0, 40))}
                </div>
                <div class="dispatch-card-meta">
