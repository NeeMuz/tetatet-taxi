...
async function geocodeAddress(query) {
    ...
    const q = query.includes('Germany') || query.includes('Deutschland') ? query : `${query}, Germany`;
    const url = `${NOMINATIM}/search?format=json&limit=1&countrycodes=de&q=${encodeURIComponent(q)}`;
    try {
        const res = await fetch(url, { headers: { 'Accept-Language': 'de' } });
        if (!res.ok) return null;
        const data = await res.json();
        if (!data.length) return null;
        const result = { lat: parseFloat(data[0].lat), lng: parseFloat(data[0].lon), address: data[0].display_name };
...
class LeafletRideMap {
    ...
    _addMapControls(options) {
        ...
        const SatelliteControl = L.Control.extend({
            ...
            onAdd() {
                ...
                btn.type = 'button';
                btn.title = 'Спутник / Карта';
                btn.setAttribute('aria-label', 'Переключить вид карты');
        ...
        if (options.showLocate !== false) {
            const LocateControl = L.Control.extend({
                ...
                onAdd() {
                    ...
                    btn.innerHTML = `${LOCATE_ICON_SVG}<span>Моя локация</span>`;
                    L.DomEvent.disableClickPropagation(wrap);
                    L.DomEvent.on(btn, 'click', (e) => {
                        L.DomEvent.stop(e);
                        self.locateUser();
    ...
    setClickTarget(target) {
        ...
        if (hint) {
            hint.textContent = target === 'from'
                ? 'Кликните на карте — точка A (откуда)'
                : 'Кликните на карте — точка B (куда)';
        }
    }
    ...
    _setPickup(lat, lng, address) {
        ...
        this.fromMarker = L.marker([lat, lng], { icon: markerIcon('A', false), draggable: true }).addTo(this.map);
    ...
    _setDropoff(lat, lng, address) {
        ...
        this.toMarker = L.marker([lat, lng], { icon: markerIcon('B', true), draggable: true }).addTo(this.map);
    ...
    async _buildRouteFromMarkers(adjustZoom) {
        ...
        this.routeLine = L.polyline(route.coords, { color: '#ffffff', weight: 5, opacity: 0.9 }).addTo(this.map);
    ...
    showCar(progress = 0.4) {
        ...
        this.carMarker = L.marker(latlngs[idx], {
